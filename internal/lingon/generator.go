...
// exportToTerraform exports the stack to Terraform code
// This is where Lingon integration would happen - generating actual .tf files
func exportToTerraform(stack *Stack) ([]byte, error) {
	// TODO: Implement actual Lingon Terraform generation
	// For now, return a placeholder that shows the structure

	output := fmt.Sprintf(`# Generated by Forge using Lingon
# Service: %s
# Region: %s
...
`,
		...
		stack.Provider.Region,
		lo.Ternary(stack.Provider.Profile != "", fmt.Sprintf("profile = \"%s\"", stack.Provider.Profile), ""),
		len(stack.Functions),
		O.IsSome(stack.APIGateway),
		len(stack.Tables),
		len(stack.EventBridgeRules),
		len(stack.StateMachines),
		len(stack.Topics),
		len(stack.Queues),
		len(stack.Buckets),
		len(stack.Alarms),
