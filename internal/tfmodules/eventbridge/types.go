...
func (m *Module) WithECSTarget(ruleKey, clusterARN, taskDefinitionARN string, subnets []string) *Module {
	// Defensive copy to prevent shared mutable state
	subnetsCopy := make([]string, len(subnets))
...
// WithEventPatternRule creates a rule with an event pattern.
...
func (*Module) Configuration() (string, error) {
